...
(async () => {
  ...
  try {
    ...
    await evaluate(`openMyStatusModal('notifications')`);
    await until(`document.querySelector('.my-notification-intro')`);
    assert.ok((await evaluate(`document.querySelector('.my-notification-intro').textContent`)).includes('临期与超期请在“我的待办”查看'));
    assert.equal(await evaluate(`document.querySelectorAll('.notification-kind.kind-due_soon, .notification-kind.kind-overdue').length`), 0);
    await evaluate(`document.querySelector('#modal-cancel').click()`);
    await until(`document.querySelector('#modal-mask').classList.contains('hidden')`);
    ...
    await evaluate(`(() => {
      ...
      const owner = body._owner.querySelector('input[value="系统管理员"]');
      owner.checked = true;
