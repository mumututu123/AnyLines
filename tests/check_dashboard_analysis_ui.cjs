...
(async () => {
  ...
  try {
    ...
    assert.equal(await evaluate(`document.querySelectorAll('.analysis-delay-gantt .analysis-star-ghost').length`), 1);
    assert.ok((await evaluate(`document.querySelector('.analysis-delay-gantt svg').getAttribute('aria-label')`)).includes('延期传递方向'));
