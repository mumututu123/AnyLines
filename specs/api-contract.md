...
### 2.5 OpenAPI 与 Swagger UI

| 方法 | 路径 | 结果 | 权限 |
| --- | --- | --- | --- |
| GET | `/api/openapi.json` | OpenAPI 3.0 文档，覆盖全部 `/api/` 路由、字段结构及请求响应样例 | 公开读取 |

Swagger UI 页面位于 `/api-docs/`，用户可从头像菜单“操作审计”下方的“API 接口”打开。
页面静态资源随项目本地部署，不依赖运行时外网；调试请求携带同源 HttpOnly Cookie，并直接遵循当前账号、当前空间和归档状态约束。执行写请求会修改真实业务数据。
